...
public class WeightedRandomizer : MonoBehaviour
{
    ...
    public GameObject itemPrefab; // Assign in the Unity Editor (the item to instantiate)
    public int baseMaxItems = 100; // Base number of items
    private int maxItems; // Dynamically adjusted based on time
    public float timeThreshold = 8.0f; // Time threshold in minutes

    void Start()
    {
        // Adjust maxItems based on time
        AdjustMaxItemsBasedOnTime("timer.txt");
        ...
        foreach (var room in roomTriggerZones)
        {
            ...
            if (itemsToSpawn.ContainsKey(roomName) && roomPositions.ContainsKey(roomName))
            {
                ...
                for (int i = 0; i < itemCount; i++)
                {
                    ...
                    // Instantiate item at the specific position
    ...
    void AdjustMaxItemsBasedOnTime(string filePath)
    {
        ...
        if (File.Exists(path))
        {
            ...
            foreach (var line in lines)
            {
                ...
                if (parts.Length == 2 && parts[0] == "TimeElapsed")
                {
                    float timeElapsed = float.Parse(parts[1]);
